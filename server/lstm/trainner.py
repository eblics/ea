...
flags.DEFINE_bool('debug', True, 'More debug info in Tensorboard')
...
flags.DEFINE_string("model_path",'/model/model-7','')
flags.DEFINE_string("pi_path",'','用来预测的输入')
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, debug=False):
    ...
    self._input_data = tf.placeholder(tf.float32, [batch_size, num_steps,1])
    ...
    rnn = CudnnLSTM(config.num_layers, size, size, input_mode='linear_input', direction='unidirectional',
                                         dropout=config.keep_prob, seed=0)
    params_size_t = rnn.params_size()
    self._initial_input_h = tf.placeholder(data_type(is_lstm_layer=True), shape=[config.num_layers,config.num_steps, size]) #self._initial_input_h = tf.Variable(tf.zeros([config.num_layers, batch_size, size]))
    self._initial_input_c = tf.placeholder(data_type(is_lstm_layer=True), shape=[config.num_layers,config.num_steps, size]) #self._initial_input_c = tf.Variable(tf.zeros([config.num_layers, batch_size, size]))
    ...
    output, output_h, output_c = rnn(is_training=is_training, input_data=inputs, input_h=self.input_h,input_c=self.input_c, params=self.params)
    self._output_h = output_h
    self._output_c = output_c
    # output = tf.reshape(tf.transpose(outputs, [1, 0, 2]), [-1, size])

    softmax_w = tf.get_variable("softmax_w", [batch_size,size,1], dtype=data_type(is_lstm_layer=False))
    softmax_b = tf.get_variable("softmax_b", [batch_size,num_steps], dtype=data_type(is_lstm_layer=False))
...
def run_epoch(session, model, data, eval_op=None, verbose=False, epoch_ind=0):
    ...
    h = np.zeros(shape=(model.num_layers, model.num_steps, model.size), dtype=np.float32)
    c = np.zeros(shape=(model.num_layers, model.num_steps, model.size), dtype=np.float32)
    for step,(x,y)in enumerate(reader.ea_input_from_raw(data, model.batch_size, model.num_steps)):
        # x,y=reader.ea_input_from_raw(data, model.batch_size, model.num_steps)
        feed_dict = {}
        ...
        if verbose and step % (epoch_size // 10) == 10:
            print("cost %.3f %.3f perplexity: %.3f speed: %.0f wps" %
                  (cost,step * 1.0 / epoch_size, np.exp(costs / iters),
...
def predict():
    ...
    with tf.Graph().as_default():
        ...
        # initializer = tf.random_uniform_initializer(-1*FLAGS.init_scale,
                # FLAGS.init_scale)
        # with tf.variable_scope("Model", reuse=None, initializer=initializer):
        m = PTBModel(is_training=False, config=config, debug=FLAGS.debug)
        with tf.Session() as session:
            ...
            # sv.saver.restore(session, FLAGS.model_path)
            for step,(x,y)in enumerate(reader.ea_input_from_raw(test_data, m.batch_size, m.num_steps)):
                if step>10:
                    break;
                # fetches = [m._logits, m.output_h, m.output_c]
                # print('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
                # print(x)
                # print('yyyyyyyyyyyyyyyyyyyyyyyyyyyyyy')
                # print(y)
                fetches=[m.logits,m._cost]
                h = np.zeros(shape=(m.num_layers, m.num_steps, m.size), dtype=np.float32)
                c = np.zeros(shape=(m.num_layers, m.num_steps, m.size), dtype=np.float32)
                feed_dict = {}
                feed_dict[m.input_data] = x
                feed_dict[m._targets] = y
                feed_dict[m.input_c] = c
                feed_dict[m.input_h] = h
                # logits,oh,oc=session.run(fetches,feed_dict)
                logits,loss=np.array(session.run(fetches,feed_dict))
                logits=np.reshape(logits,[-1])
                print("o:%f t:%f"%(logits[-1],y[-1][-1]))
                print(loss)


def main(_):
    if(sys.argv[1]=='train'): train()
    elif sys.argv[1]=='predict':predict()
