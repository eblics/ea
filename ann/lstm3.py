...
num_steps= 20     #时间步
num_layers=2
num_units= 7    #hidden layer units
num_trtimes=5
num_epochs=10000
keep_prob=0.5
...
yi=[]
# print(len(df))
for i in range(num_steps,len(df)):
# for i in range(PERIOD,PERIOD+10):
    item=[]
    item.append(df['open'][i-num_steps:i].tolist())
    item.append(df['close'][i-num_steps:i].tolist())
    # item.append(df['timestamp'][i-PERIOD:i].tolist())
    lable=[df.iloc[i]['close']]
    item=np.array(item)
...
Y=tf.placeholder(tf.float32, [None,1]) #每批次tensor对应的标签
...
#===============train begin=============================#yy
            ...
            cost=0
            try:
                _,cost_=sess.run([train_op,loss],feed_dict={LR:lr,X:xi,Y:yi})
                iters+=1
                cost+=cost_
            except ValueError:
                print('err')
